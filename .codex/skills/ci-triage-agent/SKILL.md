...
- Use `$ci-triage-agent` to check the repo's open PRs and report broken CI.
- Run scheduled CI triage and log anything currently failing.
- Use `$ci-triage-agent` to fix trivial dependency-update CI failures.
...
- Do not run as a long-lived daemon from inside Codex. This skill supports one scheduled invocation at a time.
