...
`scripts/run_training.sh` is flag-driven — run `scripts/run_training.sh --help` for the full list. Beyond the wrapper-only `--cargo-profile release|dev` (default `release`; `dev` for faster debug builds), it accepts `--model`, `--checkpoint`, `--tokenizer`, `--train-tokenizer`, `--vocab-size`, `--train-steps`, `--eval-interval`, `--prefetch-batches`, `--log-format`, `--benchmark*`, and `--artifacts-dir`. It defaults to `cargo run --release`; for `--backend cuda` it defaults to JSON logs, `--prefetch-batches 2`, and `--eval-interval 500` unless those flags are passed. The old `RUSTY_GPT_*` environment overrides for this script are **deprecated** — still honored (the matching flag wins), but each emits a deprecation warning; prefer the flags.
